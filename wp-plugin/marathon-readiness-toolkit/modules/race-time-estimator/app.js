(() => {
  const root = document.querySelector("#race-time-estimator");
  if (!root) return;


  // ---- DOM ----
  const distanceEl = root.querySelector("#distance-inputs");
  ...
  const btnEl = root.querySelector("#calculate-btn");
  const outputValueEl = root.querySelector("#output-value");
  const timeGroupEl = root.querySelector("#race-time-estimator .rte-time");
