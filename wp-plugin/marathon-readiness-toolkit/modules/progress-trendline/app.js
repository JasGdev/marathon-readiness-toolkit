...
function readPaceFromInputs(minEl, secEl, { allowBlank = true } = {}) {
...
function writePaceToInputs(minEl, secEl, paceSec) {
...
clearBtn?.addEventListener('click', async () => {
	...
	if (goalMinEl) goalMinEl.value = '';
	if (goalSecEl) goalSecEl.value = '';
	if (editGoalMinEl) editGoalMinEl.value = '';
	if (editGoalSecEl) editGoalSecEl.value = '';

	// Clear check-in pace inputs
	if (paceMinEl) paceMinEl.value = '';
	if (paceSecEl) paceSecEl.value = '';
...
saveSettingsBtn?.addEventListener('click', () => {
	...
	const goalPaceSec = readPaceFromInputs(editGoalMinEl, editGoalSecEl, {
		allowBlank: true,
	});

	const goalHasAny = !!String(editGoalMinEl?.value ?? '').trim() || !!String(editGoalSecEl?.value ?? '').trim();

	if (goalHasAny && goalPaceSec === null) {
		showError('目标配速请输入有效的 m:ss（秒数需 0–59）。例如：5:00');
		return;
	}

	state.config.raceDate = raceDate;
	state.config.goalPaceSec = goalPaceSec;
...
startBtn?.addEventListener('click', () => {
	...
	const goalSec = readPaceFromInputs(editGoalMinEl, editGoalSecEl, {
		allowBlank: true,
	});
...
calcPaceBtn?.addEventListener('click', () => {
	...
	writePaceToInputs(paceMinEl, paceSecEl, paceSec);
});
...
addBtn?.addEventListener('click', () => {
	...
	const paceSecPerKm = readPaceFromInputs(paceMinEl, paceSecEl, {
		allowBlank: false,
	});

	const paceHasAny = !!String(paceMinEl?.value ?? '').trim() || !!String(paceSecEl?.value ?? '').trim();

	if (!paceHasAny || paceSecPerKm === null) {
		showError('请输入当前可持续配速（m:ss /km）（秒数需 0–59），或使用跑步计算器自动填入。');
		return;
	}
